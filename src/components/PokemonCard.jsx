...
export const PokemonCard = ({ pokemonURL, onClick }) => {
	...
	return (
		<div
			...
			className='relative pb-3 bg-white font-semibold text-center capitalize rounded-3xl border border-transparent hover:border-gray-200 shadow-lg shadow-slate-400/10 cursor-pointer group grid gap-2'
		>
			<div className='h-9'>
				<img
			...
			<ul className='flex gap-2 justify-center'>
				{pokemon.types?.map(({ type }) => (
					<li
						className={`text-white text-sm p-1 rounded-md px-2 ${
							colorType[type.name]
						}`}
						key={type.name}
					>
